...
namespace Bam.Net.CoreServices.ApplicationRegistration.Data.Dao
{
	...
	[Serializable]
	[Bam.Net.Data.Table("ClientServerConnection", "ApplicationRegistration")]
	...
	public DateTime? Created
	{
		...
		public static ClientServerConnectionCollection LoadAll(Database database = null)
		{
            ...
            var results = new ClientServerConnectionCollection(db, sql.GetDataTable(db))
            {
                Database = db
            };
            return results;
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<ClientServerConnection>> batchProcessor, Bam.Net.Data.OrderBy<ClientServerConnectionColumns> orderBy, Database database = null)
		{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<ClientServerConnectionColumns> where, Action<IEnumerable<ClientServerConnection>> batchProcessor, Bam.Net.Data.OrderBy<ClientServerConnectionColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				ClientServerConnectionColumns columns = new ClientServerConnectionColumns();
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (ClientServerConnectionColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});
		}

		public static ClientServerConnection GetById(uint? id, Database database = null)
		{
			Args.ThrowIfNull(id, "id");
			Args.ThrowIf(!id.HasValue, "specified ClientServerConnection.Id was null");
		...
		public static ClientServerConnection GetById(ulong? id, Database database = null)
		{
			Args.ThrowIfNull(id, "id");
			Args.ThrowIf(!id.HasValue, "specified ClientServerConnection.Id was null");
			return GetById(id.Value, database);
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<ClientServerConnectionColumns> where, Database database = null)
		{
			SetOneWhere(where, out ClientServerConnection ignore, database);
		}
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<ClientServerConnectionColumns> where, out ClientServerConnection result, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static ClientServerConnectionCollection Top(int count, QueryFilter where, string orderBy = null, SortOrder sortOrder = SortOrder.Ascending, Database database = null)
		{
			Database db = database ?? Db.For<ClientServerConnection>();
			QuerySet query = GetQuerySet(db);
			query.Top<ClientServerConnection>(count);
