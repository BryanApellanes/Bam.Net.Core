...
namespace Bam.Net.CoreServices.ApplicationRegistration.Data.Dao
{
	...
	public OrganizationUserCollection OrganizationUsersByOrganizationId
	{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<Organization>> batchProcessor, Bam.Net.Data.OrderBy<OrganizationColumns> orderBy, Database database = null)
		{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<OrganizationColumns> where, Action<IEnumerable<Organization>> batchProcessor, Bam.Net.Data.OrderBy<OrganizationColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				OrganizationColumns columns = new OrganizationColumns();
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (OrganizationColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});
		}
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<OrganizationColumns> where, Database database = null)
		{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<OrganizationColumns> where, out Organization result, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static OrganizationCollection Top(int count, QueryFilter where, string orderBy = null, SortOrder sortOrder = SortOrder.Ascending, Database database = null)
		{
			Database db = database ?? Db.For<Organization>();
			QuerySet query = GetQuerySet(db);
			query.Top<Organization>(count);
