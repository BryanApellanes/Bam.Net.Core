...
namespace Bam.Net.CoreServices.ApplicationRegistration.Data.Dao
{
	...
	[Serializable]
	[Bam.Net.Data.Table("ConfigurationSetting", "ApplicationRegistration")]
	...
	public Configuration ConfigurationOfConfigurationId
	{
		get
		{
			...
			{
				_configurationOfConfigurationId = Bam.Net.CoreServices.ApplicationRegistration.Data.Dao.Configuration.OneWhere(c => c.KeyColumn == this.ConfigurationId, this.Database);
			}
		...
		public static ConfigurationSettingCollection LoadAll(Database database = null)
		{
            ...
            var results = new ConfigurationSettingCollection(db, sql.GetDataTable(db))
            {
                Database = db
            };
            return results;
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<ConfigurationSetting>> batchProcessor, Bam.Net.Data.OrderBy<ConfigurationSettingColumns> orderBy, Database database = null)
		{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<ConfigurationSettingColumns> where, Action<IEnumerable<ConfigurationSetting>> batchProcessor, Bam.Net.Data.OrderBy<ConfigurationSettingColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				ConfigurationSettingColumns columns = new ConfigurationSettingColumns();
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (ConfigurationSettingColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});
		}
		...
		public static ConfigurationSetting GetById(uint? id, Database database = null)
		{
			...
			Args.ThrowIf(!id.HasValue, "specified ConfigurationSetting.Id was null");
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<ConfigurationSettingColumns> where, Database database = null)
		{
			SetOneWhere(where, out ConfigurationSetting ignore, database);
		}
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<ConfigurationSettingColumns> where, out ConfigurationSetting result, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static ConfigurationSettingCollection Top(int count, QueryFilter where, string orderBy = null, SortOrder sortOrder = SortOrder.Ascending, Database database = null)
		{
			Database db = database ?? Db.For<ConfigurationSetting>();
			QuerySet query = GetQuerySet(db);
			query.Top<ConfigurationSetting>(count);
