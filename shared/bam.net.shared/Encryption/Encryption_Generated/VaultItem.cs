...
namespace Bam.Net.Encryption
{
	...
	[Serializable]
	[Bam.Net.Data.Table("VaultItem", "Encryption")]
	public partial class VaultItem: Bam.Net.Data.Dao
	{
		...
		public VaultItem(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
	...
	public Vault VaultOfVaultId
	{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<VaultItem>> batchProcessor, Bam.Net.Data.OrderBy<VaultItemColumns> orderBy, Database database = null)
		{
		...
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<VaultItemColumns> where, Action<IEnumerable<VaultItem>> batchProcessor, Bam.Net.Data.OrderBy<VaultItemColumns> orderBy, Database database = null)
		{
			await Task.Run(async ()=>
			{
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (VaultItemColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});
		}

		public static VaultItem GetById(uint? id, Database database = null)
		{
			Args.ThrowIfNull(id, "id");
			Args.ThrowIf(!id.HasValue, "specified VaultItem.Id was null");
		...
		public static VaultItem GetById(ulong? id, Database database = null)
		{
			Args.ThrowIfNull(id, "id");
			Args.ThrowIf(!id.HasValue, "specified VaultItem.Id was null");
			return GetById(id.Value, database);
		...
		public static VaultItem GetByUuid(string uuid, Database database = null)
		{
			return OneWhere(c => Bam.Net.Data.Query.Where("Uuid") == uuid, database);
		}

		public static VaultItem GetByCuid(string cuid, Database database = null)
		{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<VaultItemColumns> where, Database database = null)
		{
			SetOneWhere(where, out VaultItem ignore, database);
		}
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static void SetOneWhere(WhereDelegate<VaultItemColumns> where, out VaultItem result, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static VaultItemCollection Top(int count, QueryFilter where, Database database)
		{
			return Top(count, where, null, database);
		}
		/// <summary>
		...
		[Bam.Net.Exclude]
		public static VaultItemCollection Top(int count, QueryFilter where, string orderBy = null, SortOrder sortOrder = SortOrder.Ascending, Database database = null)
		{
			Database db = database ?? Db.For<VaultItem>();
			QuerySet query = GetQuerySet(db);
			query.Top<VaultItem>(count);
